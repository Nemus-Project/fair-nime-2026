...
There has been a concerted effort to address the problem of archiving and open sourcing of projects at NIME over the past 10 years. \citet{mcpherson_nimehub_2016} from 2016 was the first discussion on open research practice, the intention to create a a central repository (or database) for DMI designs. Part of that workshop's primary eight goals were the FAIR adjacent aims of encouraging reproducibility, open access and encouraging a normalised practice of documentation. The NIMEhub did not ultimately manifest, but the workshop has sparked further discussion of open research at NIME \cite{jensenius_open_2019, jensenius_nime_2020}.
The focus of open research at NIME has tended towards methodologies applicable to open hardware \citet{calegario_documentation_2021, fiordelmondo_towards_2025}, the provision of making electronics designs accessible allowing fabrication outside of a central manufacturer.
...
There were five instances where the repository was unreachable from the link, either requiring authentication or a private repository, the repository was completely empty or a link was missing but a web search of the project name yielded the result in the first page.
The authors were contacted and four out of five have provided an alternative at time of writing.
These repositories all qualified, but illustrate the potential pitfalls in an unstandardised system for reference project repositories.
...
Overwhelmingly GitHub is the preferred repository platform with 30 projects hosting a repository there, which was the trend identified in \citet{fiordelmondo_towards_2025}. Table~\ref{tab:git_placement} shows how authors from NIME 2025 chose to disseminate source URLs and by far the  most common is a URL in a footnote. Broken down further, 7 papers provided a URL in footnote on the first page an remaining 11 on a later page.
...
Each directory was search for files in which there was an occurrence of git repository. Using the \texttt{pdfgrep} software \cite{deifel_pdfgrep_2024}, papers were searching case-insensitive regular expression \texttt{\textbackslash Wgit},
which captures `git' and variants like `\textbf{Git}Hub' and `\textbf{Git}Lab' but also URLs  like \texttt{https:/\textbf{/git}lab.com/}. A deviation from \cite{fiordelmondo_towards_2025} was to include \texttt{github.io} webpages in the results as it would suggest usage or familiarity with GitHub and git software. The list of papers were in which there was a match was counted for each year and shown in Table~\ref{tab:git_occurences}.
