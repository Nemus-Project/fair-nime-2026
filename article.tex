...
Search carried out a manual keyword search each paper on each paper from NIME 2025, much in the same manner as  in \cite{fiordelmondo_longevity_2024} Review A . The keywords searched were `git', `source', `doi', `open', `http'. Upon failing to find any result, the paper was manually scan read to identify alternate methods of including a reference to a source repository.
...
Of the 96 papers from NIME 2025, 28 papers have a link to some kind of source in them though 50 papers make reference to `git,' `github,' or `gitlab.' Of the 96 papers only one \cite{studley_swimtunes_2025} has a digital archive with a DOI.
...
NIME provides a space in it's archive entry from which the DOI is taken. Of the 96, only 10 papers have supplementary files and these are exclusively videos.
...
Archiving should provide a DOI with which to cite. Repositories may also provide a recommended citation, but this should very much be looked upon as a bandage to a broken system and not a solution. The very helpful citation file format provides a means to have a structure citation to software, but unhelpfully, is used to cite repository and not a specific documented version of the sofwtare (citation needed)

\subsection{limits in archiving modular projects}

Problem with a modular process is that zenodo's integration does not support it or a llow for easy configuration.
