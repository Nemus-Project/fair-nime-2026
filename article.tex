...
clear there is a desire to share work, offload the burden on choosing the most effective way to dissemintate, or the reader in locating the work, the solution would be to standarddise one, possibly two field that are in the papers template text, and in the metadata.
...
\begin{figure}
    ...
    \caption{Caption}
    \label{fig:Project details as a standard field in the paper template would allow for quick identification of the project status and files. These details are not obligatory and could be witheld, but the choice to do so becomes deliberate.}
\end{figure}
...
\centering
    \begin{tikzpicture}[
      box/.style = {draw, minimum width=0.2\linewidth, minimum height=1.5cm, align=center},
    ...
    \node[box] (box1) {Firmware};
    \node[box, right=of box1] (box2) {Schematics};
    \node[box, right=of box2] (box3) {Enclosure};
    \node[box, below=2cm of box2] (bottom) {Interface};
